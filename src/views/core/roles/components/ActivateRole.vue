...
const submit = async () => {
  loading.value = true;
  response.value = await rolesStore.activate(props.record_id, form.value)
...
const validateForm = computed(() => {
  let errors = 0;
  if (form.value.confirm_activation === undefined || form.value.confirm_activation === false)
    errors++;
  ...
  <ModalComponent
    ...
    <div class=" space-y-5">
      ...
      <form class="space-y-5">
        ...
        <CheckboxField
            v-model="form.confirm_activation"
          :id="'confirm_deactivation'"
          :false_value="false"
          :true_value="true"
          :error="errors.confirm_activation"
          :title="`Are you sure you want to activate role <b>${record.name}</b>?`"
        />
      </form>
