...
const init = async () => {
  const response = await userStore.getAllRoles()
  if (response.status) {
    roles.value = response.data
  }

  form.value.roles = roles.value.map((role) => {
    let user_roles = props.user.user_roles.filter((item) => item.id === role.id)
    return {id:role.id, selected: user_roles.length > 0}
  })
}
...
const page = {
  id: 'manage_user_roles_drawer',
  title: 'Manage User Roles',
  button_icon: 'fa fa-briefcase',
}
...
const submit = async () => {
  loading.value = true;
  response.value = await userStore.manageUserRoles(props.user_id, form.value)
        ...
        <table class="is-zebra w-full text-left"
          ...
          <tr v-for="(role, index) in roles" :key="index">
            <td class="whitespace-nowrap border border-l-0 border-slate-200 px-3 py-3 dark:border-navy-500 lg:px-5">
              <CheckboxField
                  :value="role.id"
                  v-model="form.roles[index].selected"
                  :id="role.name"
                  :error="null"
                  :title="''"
                  :multiple="true"
            ...
            <td class="border border-t-0 border-l-0 border-slate-200 px-3 py-3 text-slate-800 dark:border-navy-500 dark:text-navy-100 lg:px-5">
              <div class=" font-semibold  uppercase">{{ role.title }}</div>
              <small>
                {{ role.description }}
              </small>
            </td>
