
const projects = [
    {
        id:1,
        color: '#ff9800',
        image: '/src/assets/images/800x600.png',
        created_date: 'May 01, 2021',
        name: 'Mobile App',
        category: 'Prototyping',
        progress: 78,
        duration_left: '1 week left',
        assigned: [
            { name: 'Jane Doe', profile_pic: '/src/assets/images/200x200.png'},
            { name: 'Jane Doe', profile_pic: null },
            { name: 'Jane Doe', profile_pic: '/src/assets/images/200x200.png'},
            { name: 'Jane Doe', profile_pic: '/src/assets/images/200x200.png'},
            { name: 'Jane Doe', profile_pic: null}
        ]
    },
    {
        id:2,
        color: "'rgb(14 165 233 / .15)'",
        image: '/src/assets/images/800x600.png',
        created_date: 'June 04, 2021',
        name: 'Design Learn Management System',
        category: 'UI/UX Design',
        progress: 25,
        duration_left: '2 Weeks left',
        assigned: [
            { name: 'Jane Doe', profile_pic: '/src/assets/images/200x200.png'},
            { name: 'Jane Doe', profile_pic: null},
            { name: 'Jane Doe', profile_pic: '/src/assets/images/200x200.png'},
            { name: 'Jane Doe', profile_pic: '/src/assets/images/200x200.png'},
            { name: 'Jane Doe', profile_pic: '/src/assets/images/200x200.png'},
            { name: 'Jane Doe', profile_pic: null}
        ]
    },
    {
        id:3,
        color: 'bg-secondary/15',
        image: '/src/assets/images/illustrations/chat-ui.svg',
        created_date: 'Oct 27, 2021',
        name: 'Chat Mobile App',
        category: 'Prototyping',
        progress: 52,
        duration_left: '6 Weeks left',
        assigned: [
            { name: 'Jane Doe', profile_pic: '/src/assets/images/200x200.png'},
            { name: 'Jane Doe', profile_pic: null},
            { name: 'Jane Doe', profile_pic: '/src/assets/images/200x200.png'},
            { name: 'Jane Doe', profile_pic: '/src/assets/images/200x200.png'},
            { name: 'Jane Doe', profile_pic: null}
        ]
    },
    {
        id:4,
        color: 'bginfo',
        image: 'image',
        created_date: 'Sep 16, 2021',
        name: 'Store Dashboard',
        category: 'UI/UX Design',
        progress: 33,
        duration_left: '3 Weeks left',
        assigned: [
            { name: 'Jane Doe', profile_pic: '/src/assets/images/200x200.png'},
            { name: 'Jane Doe', profile_pic: null},
            { name: 'Jane Doe', profile_pic: '/src/assets/images/200x200.png'},
            { name: 'Jane Doe', profile_pic: '/src/assets/images/200x200.png'},
            { name: 'Jane Doe', profile_pic: null}
        ]
    }
]

export default projects