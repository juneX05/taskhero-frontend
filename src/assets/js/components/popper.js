...
export class PopperMultiple {

  constructor(wrapper, ref, box, options, evt = "click", ontoggle = () => {}) {
    this.wrapper = wrapper;
    this.wrapperElements = document.querySelectorAll(wrapper);
    this.wrapperElements.forEach((element, index) => {
      element.setAttribute('id', 'popper_multiple_' + index);
      new Popper('#' + element.id, ref, box, options, evt, ontoggle)
    })
  }
}
